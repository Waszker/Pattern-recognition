...
def randomforest_identification(is_debug = False, trees=50, normalize=False):
    """
...
def randomforest_classification(trees=50, normalize=False):
...
def rf_identify_points(points, labels, trees=100, normalize=False):
    result_matrix = np.zeros((11, 11))
    train_points, _, class_labels, _ = _load_training_and_test_sets(normalize)

    # RF routine
    vectors = RandomForestClassifier(n_estimators=trees)
...
def rf_classify_points(trees=100, normalize=False):
    norm = None
    if normalize == True:
        norm = loading.get_normalize_vector()
    train_points, test_points, class_labels, test_labels = _load_training_and_test_sets(normalize)
